...
// 팀장일 때만 '크루관리' 탭이 추가로 붙는다 (가입요청 승인·강퇴는 팀장 전용 화면으로 분리).
function getCrewPageTabs() {
  // 공지는 이제 별도 탭이 아니라 크루 메인 화면 안에 카드로 들어가 있다(renderCrewNoticeCard).
  const tabs = ['크루 메인', '크루채팅', '크루대전', '크루원 정보'];
  if (getMyCrewRole() === '팀장') tabs.push('크루관리');
  return tabs;
}
...
async function loadJoinableCrews() {
  try {
    ...
    JOINABLE_CREWS = body.data.map(c => {
      ...
      return {
        id: c.id, name: c.name, desc: c.description, concepts: c.concepts || [],
...
function renderCrew() {
  ...
  ${activeTab === '크루 메인' ? renderCrewOverview()
      : activeTab === '크루채팅' ? renderCrewChat()
        : activeTab === '크루대전' ? renderCrewBattleTab()
...
function renderCrewCreate() {
  ...
  <div class="card" style="max-width:480px;margin:0 auto;">
    <p class="section-label">새 크루 만들기 (포인트 1000 소모)</p>
    <div class="field">
      <label>활동 지역</label>
    ...
    <button class="btn btn-primary btn-block" onclick="createCrew()">1000P로 크루 생성</button>
  </div>`;
}
async function createCrew() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/crews`, {
      ...
      body: JSON.stringify({ name, description: desc, concepts: state.crew.concepts })
    });
    const body = await res.json();
    if (!body.success) { toast(body.message || '크루 생성에 실패했습니다'); return; }

    await loadMyProfile(); // 포인트 1000 차감된 실제 값 반영
    await loadMyCrew();
...
function renderCrewOverview() {
  // 크루대전에 실제로 참여해 점수를 쌓은 크루원만 표시한다(GET .../battle-contributions —
  // 아직 대전에 한 번도 안 나간 크루원은 여기 안 잡힌다. 전체 크루원 목록은 members/크루원 정보 탭).
  const battleContribs = state.crew.battleContributions || [];
  const battleTotal = battleContribs.reduce((s, c) => s + c.totalScore, 0) || 1;
  const battleRanked = battleContribs.map((c, i) => ({ ...c, rank: i + 1, pct: Math.round(c.totalScore / battleTotal * 100) }));
  ...
  <div class="grid grid-fixed-2" style="align-items:stretch;">
    ...
    <div class="card">
      <p class="section-label">크루대전 기여도</p>
      ${battleRanked.length ? battleRanked.map(c => `
        <div class="rep-row">
          <span class="rank-num ${c.rank === 1 ? 'top' : ''}" style="min-width:24px;height:22px;">${c.rank}</span>
          <span class="user-avatar" style="width:22px;height:22px;font-size:10px;flex:none;background:${avatarColor(c.rank - 1)}">${avatarInitial(c.nickname)}</span>
          <span style="width:64px;">${c.nickname}${Number(c.userId) === Number(state.user.id) ? ' <span class="pill pill-accent">나</span>' : ''}${!c.currentMember ? ' <span class="pill pill-muted">탈퇴</span>' : ''}</span>
          <div class="bar-track"><span style="width:${c.pct}%;background:var(--accent)"></span></div>
          <span class="angle mono">${c.totalScore.toLocaleString()}점</span>
        </div>`).join('') : `<p class="empty-note" style="margin-top:8px;">아직 크루대전에 참여한 기록이 없어요.</p>`}
    </div>
  </div>
  <div class="card" style="margin-top:14px;">
    <p class="section-label">이번주 크루미션</p>
    <div class="flex-between"><h3 style="margin:0;">개인운동에서 ${gm.ex} 하기</h3><span class="pill pill-accent">주간</span></div>
...
/* ---------- 크루대전 탭: 원래 크루메인 하단에 있던 크루대전 카드를 별도 탭으로 분리 ---------- */
function renderCrewBattleTab() {
  ...
  return `
  <div class="card crew-battle-card">
    <div class="flex-between">
      ...
      ${renderCrewBattleServerActions()}
    </div>
  </div>
  ${renderCrewBattleSetupPanel()}
  ${renderCrewBattleHistory()}
  ${renderCrewBattleDemoButtons()}`;
}
...
function renderCrewChat() {
  ...
  <div class="chat-wrap" style="max-width:860px;margin:0 auto;">
    <div class="chat-log" id="crew-chat-log">
      ${msgs.map(m => `
        <div class="bubble ${m.mine ? 'me' : 'them'}" ${(!m.mine && m.senderId != null) ? `style="cursor:pointer;" title="클릭해서 신고/차단" onclick="openChatModeration(${m.id ?? 'null'}, ${m.senderId}, '${(m.who || '').replace(/'/g, "\\'")}')"` : ''}>
...
function appendChatBubble(m) {
  ...
  // renderCrewChat()의 초기 렌더와 동일하게, 실시간으로 쌓이는 말풍선도 클릭하면 신고/차단
  // 팝업이 뜨도록 맞춰준다(여기는 문자열 템플릿이 아니라 DOM을 직접 만드므로 onclick 속성
  // 대신 addEventListener로 연결한다).
  if (!m.mine && m.senderId != null) {
    div.style.cursor = 'pointer';
    div.title = '클릭해서 신고/차단';
    div.addEventListener('click', () => openChatModeration(m.id ?? null, m.senderId, m.who || ''));
  }
  log.appendChild(div);
  ...
  crewStompClient.connect(
    ...
    () => {
      crewChatSubscription = crewStompClient.subscribe(`/topic/crews/${state.crew.id}/chat`, (frame) => {
        ...
        const msg = { id: m.id, senderId: m.senderId, who: m.senderNickname, mine: false, text: m.text };
...
async function handleCrewMemberEvent(ev){
  if(ev.type === 'KICKED'){
    ...
    if(ev.targetUserId === state.user.id){
      toast('크루장 권한을 넘겨받았습니다');
    } else {
      toast(`${ev.targetNickname}님이 새 크루장이 되었습니다`);
    }
    await loadMyCrew(); // 내 role(팀장/크루원)과 멤버 목록의 역할 표시를 새로 받아온다
    render();
...
async function loadCrewChatHistory(){
  ...
  try{
    ...
    state.crew.chat.messages = body.data.map(m => ({
      id: m.id, senderId: m.senderId, who: m.senderNickname, mine: m.senderId===state.user.id, text: m.text
    }));
    render();
  }catch(err){
...
/* ---------- 크루채팅 메시지 신고/차단 ----------
   다른 크루원의 말풍선을 클릭하면 뜨는 작은 팝업. 백엔드에 신고/차단 API·테이블이 아직
   없어서(CrewChatController/CrewChatMessage 등을 확인했지만 관련 기능이 전혀 없음) 지금은
   화면만 있고 실제로 서버에 저장되지는 않는다 — 눌렀을 때 저장된 것처럼 속이지 않고
   아직 연동 전이라는 걸 그대로 알려준다. */
function openChatModeration(messageId, senderId, nickname) {
  if (senderId == null || Number(senderId) === Number(state.user.id)) return; // 내 메시지는 신고/차단 대상이 아니다
  state.chatModeration = { open: true, messageId, targetUserId: senderId, targetNickname: nickname };
  render();
}
...
function reportChatMessage() {
  toast('신고 기능은 아직 백엔드 연동 전이에요. 급한 건이면 고객센터로 문의를 남겨주세요.');
  closeChatModeration();
}
function blockChatUser() {
  toast('차단 기능은 아직 백엔드 연동 전이에요.');
  closeChatModeration();
}
function renderChatModerationModal() {
  const s = state.chatModeration;
  return `
  <div class="confirm-backdrop" onclick="if(event.target===this) closeChatModeration()">
    <div class="confirm-box" style="text-align:center;">
      <h3>${s.targetNickname}</h3>
      <p class="desc" style="margin:6px 0 16px;">이 크루원의 메시지를 신고하거나 차단할 수 있어요.</p>
      <div style="display:flex;flex-direction:column;gap:8px;">
        <button class="btn btn-danger" onclick="reportChatMessage()">🚨 메시지 신고하기</button>
        <button class="btn btn-secondary" onclick="blockChatUser()">🚫 이 크루원 차단하기</button>
        <button class="btn btn-ghost" onclick="closeChatModeration()">취소</button>
      </div>
    </div>
  </div>`;
}
...
/* ========================================================================
   ...
   카메라·MediaPipe 판정(exRegisterRep)에서 그대로 받아오고, 나머지 4명의 크루원과
...
function updateBattleUI(bumpedKey, delta) {
  ...
  // 카메라 화면 위에 떠 있는 실시간 스코어 오버레이도 같이 갱신한다.
...
/* ---------- 크루원 정보: 조회 전용 (양도/추방은 크루관리 탭에서 처리) ---------- */
    ...
    <table>
      ...
      <tbody>
        ${state.crew.members.map(m => `
          <tr>
            <td>${m.n}${Number(m.userId) === Number(state.user.id) ? ' <span class="pill pill-accent">나</span>' : ''}</td>
...
// 크루장 권한 양도 — 백엔드가 PATCH /me/leader/{id}로 이미 지원한다(대상은 반드시 일반 크루원,
// 자기 자신 불가). 성공하면 서버가 /topic/crews/{id}/members에 LEADER_CHANGED를 브로드캐스트해서
// handleCrewMemberEvent()가 나를 포함한 크루원 전원의 화면(역할 표시 등)을 갱신한다.
function confirmTransferCrewLeadership(targetUserId, targetName) {
  askConfirm(
    '크루장 양도',
    `${targetName}님에게 크루장을 양도할까요? 양도하면 나는 일반 크루원이 됩니다.`,
    () => { closeConfirm(); transferCrewLeadership(targetUserId); },
    '양도',
    true
  );
}
async function transferCrewLeadership(targetUserId) {
  try {
    const res = await fetch(`${API_BASE}/api/crews/me/leader/${targetUserId}`, {
      method: 'PATCH',
      headers: { 'Authorization': 'Bearer ' + state.token }
    });
    const body = await res.json();
...
// 크루원 추방 — 기존 kickMember()(크루관리 탭에서 이미 씀)를 확인창으로 감싸서 재사용한다.
function confirmKickMember(targetUserId, targetName) {
  askConfirm(
    '크루원 추방',
    `${targetName}님을 크루에서 추방할까요?`,
    () => { closeConfirm(); kickMember(targetUserId); },
    '추방',
    true
  );
}
/* ---------- 크루관리: 팀장 전용 — 크루 소개 수정 + 가입요청 승인 + 크루장 양도·강퇴 ---------- */
function renderCrewManage() {
  ...
  <p class="section-label">크루원 관리</p>
  <div class="table-wrap">
    <table>
      ...
      <tbody>
        ${state.crew.members.map(m => `
          <tr>
            ...
            <td><span class="pill ${m.role === '팀장' ? 'pill-gold' : 'pill-muted'}">${crewRoleLabel(m.role)}</span></td>
            <td class="mono">Lv.${m.level}</td>
            <td>${m.userId !== state.user.id ? `
              <div style="display:flex;gap:6px;">
                <button class="btn btn-sm btn-secondary" onclick="confirmTransferCrewLeadership(${m.userId}, '${m.n}')">양도</button>
                <button class="btn btn-sm btn-danger" onclick="confirmKickMember(${m.userId}, '${m.n}')">추방</button>
              </div>` : ''}</td>
          </tr>`).join('')}
      </tbody>
    </table>
...
async function updateCrewDesc() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/crews/me`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + state.token },
      body: JSON.stringify({ description: v, concepts: state.crew.concepts })
    });
    const body = await res.json();
    if (!body.success) { toast(body.message || '저장에 실패했습니다'); return; }
    state.crew.desc = body.data.description;
    state.crew.concepts = body.data.concepts || [];
    toast('크루 소개를 저장했습니다');
    render();
  } catch (err) {
    toast('서버에 연결할 수 없습니다');
...
async function loadCrewRegionRanking() {
  const cities = Object.keys(REGION_DATA);
  const isAll = state.crew.rankCity === RANK_ALL_CITY;
  const regionQuery = isAll ? '' : (() => {
    ...
    return `city=${encodeURIComponent(city)}&gu=${encodeURIComponent(gu)}`;
  })();
  try {
    const res = await fetch(`${API_BASE}/api/rankings/crew?${regionQuery}`);
    const body = await res.json();
    if (!body.success) return;
...
function renderCrewRegionRank() {
  ...
  // "전체"면 동 단위로 다시 나눌 기준이 없으니, 구 단위(전국 전체)로 받아온 목록을 그대로
  // 순위만 다시 매겨서 보여준다 — 동 필터는 애초에 서버가 아니라 프론트에서 city+gu 응답을
  // 동으로 한 번 더 나눈 것이라(loadCrewRegionRanking 주석 참고), "전체"엔 적용할 동 자체가 없다.
  const rankCity = isAll ? RANK_ALL_CITY : (REGION_DATA[state.crew.rankCity] ? state.crew.rankCity : cities[0]);
  const rankGus = isAll ? [] : Object.keys(REGION_DATA[rankCity]);
  const rankGu = isAll ? null : (REGION_DATA[rankCity][state.crew.rankGu] ? state.crew.rankGu : rankGus[0]);
  const dongs = isAll ? [] : REGION_DATA[rankCity][rankGu];
  const rankDong = isAll ? null : (dongs.includes(state.crew.rankDong) ? state.crew.rankDong : dongs[0]);
  const dongRows = (isAll ? state.rank.crew : state.rank.crew.filter(r => r.region === rankDong))
    .map((r, i) => ({ ...r, rank: i + 1 }));
  const rest = dongRows.filter(r => r.rank > 3);
  return `
  <p class="section-label">${isAll ? '전국 크루 랭킹' : '동네별 크루 랭킹'}</p>
  <div class="filter-bar">
    ...
    <select onchange="setCrewRankGu(this.value)" ${isAll ? 'disabled' : ''}>
      ${isAll ? `<option>--</option>` : rankGus.map(g => `<option ${g === rankGu ? 'selected' : ''}>${g}</option>`).join('')}
    </select>
    <select onchange="setCrewRankDong(this.value)" ${isAll ? 'disabled' : ''}>
      ${isAll ? `<option>--</option>` : dongs.map(d => `<option ${d === rankDong ? 'selected' : ''}>${d}</option>`).join('')}
    </select>
  </div>
  ${dongRows.length === 0 ? `<div class="empty-note">${isAll ? '아직' : '이 동네엔 아직'} 등록된 크루가 없습니다.</div>` : `
