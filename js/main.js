...
function renderMain(){
  ...
  const todayAttendanceReward = displayStreak>=3 ? 15 : 5;
  const canClaimStreak = !state.user.attendanceRewardClaimed;
  ...
  const streakHint = state.guestMode
    ...
    : (state.user.attendanceRewardClaimed
      ? `✅ 현재 연속 출석 ${displayStreak}일 · 오늘 보상 ${todayAttendanceReward}P 지급 완료`
      : (displayStreak>=3
        ? `현재 연속 출석 ${displayStreak}일 · 기본 5P + 추가 보상 10P = 오늘 총 15P`
        : `현재 연속 출석 ${displayStreak}일 · 오늘 기본 출석 보상 5P`));
  const header = `
  <div class="view-head">
  ...
  const expToNext = isGuest ? GUEST_MAIN_SAMPLE.expToNext : Math.max(0, nextLevelExp() - currentExpValue());
  const expPct = isGuest ? GUEST_MAIN_SAMPLE.exp : levelProgressPct();
  ...
  <div class="grid glance-grid" style="align-items:stretch;">
    ...
    <div class="card" style="display:flex;flex-direction:column;">
      <div>
        ...
        <div class="attendance-reward-guide" style="margin:8px 0 2px;padding:8px 10px;border:1px solid var(--line);border-radius:10px;font-size:12px;line-height:1.6;">
          <div><b>기본 출석 보상</b> <span class="mono">5P</span></div>
          <div><b>3일 연속 출석부터</b> 추가 <span class="mono">10P</span></div>
          <div><b>3일 이상 연속 출석</b> 오늘 총 <span class="mono" style="color:var(--gold);">15P</span></div>
          <div class="hint" style="margin-top:4px;font-size:11px;">※ 출석이 끊어지면 연속 출석은 다시 1일차로 표시됩니다.</div>
        </div>
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      <div class="card" style="flex:1;display:flex;flex-direction:column;justify-content:center;">
        ...
        <p class="desc mono" style="margin:0;font-size:15px;">누적 점수 <b style="font-size:20px;">${totalScore.toLocaleString()} 점</b></p>
        <p class="desc mono" style="margin:6px 0 0;font-size:14px;">레벨업까지 <b style="color:var(--gold);font-size:16px;">${expToNext.toLocaleString()} EXP</b> 남았어요!</p>
