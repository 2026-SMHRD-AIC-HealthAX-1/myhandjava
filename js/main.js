...
function renderMain(){
  ...
  // 출석 포인트는 사실 이 버튼과 무관하게 매일 로그인할 때 자동으로 지급된다(기본 5P,
  // 연속 3일째부터는 매일 +10P가 붙어 15P) — 이 버튼은 그와 별개로 "10일 연속 출석 1회성
  // 보너스 300P"를 청구하는 용도라서, 두 보상을 각각 짧게 안내한다.
  const streakHint = state.guestMode
    ? '로그인하면 연속출석 보상을 받을 수 있어요'
    : (state.user.streakRewardClaimed ? '✅ 이번 보상을 받았어요' : '매일 출석 5P · 3일 연속부터 하루 15P(5+10) · 10일 연속 시 위 버튼으로 300P 보너스!');
  const header = `
  <div class="view-head">
  ...
  // "한눈에 보기" 2열 — 캐릭터, 출석 캘린더. 예전엔 동네랭킹·누적성과·크루 유도 카드가 세번째
  // 열에 같이 들어가 있었는데, 모바일에서 이 카드들이 히스토리보다 먼저 잔뜩 쌓여 스크롤이
  // 길어진다는 피드백으로 별도 줄(glanceRow3)로 빼고, 빠진 자리만큼 이 두 카드를 키웠다.
  const glanceGrid = `
  <div class="grid glance-grid" style="align-items:stretch;">
    <div class="card" style="text-align:center;display:flex;flex-direction:column;">
      ...
      <div style="flex:1;display:flex;flex-direction:column;justify-content:center;">
        ...
        <p class="hint" style="margin:4px 0 0;font-size:13px;">${isGuest ? 'Lv.0' : userLevelBadge(state.user.grade, state.user.gradeName, state.user.level, true)} · 꾸준함을 키우는 중</p>
      </div>
    ...
    <div class="card" style="display:flex;flex-direction:column;">
      ...
      <div style="margin-top:auto;">
        <button class="btn btn-sm ${streakActive?'btn-primary':'btn-ghost'} btn-block" ${streakActive?'':'disabled style="opacity:.5;cursor:not-allowed;"'} onclick="${state.guestMode ? "goto('login')" : 'claimStreakReward()'}">출석하기</button>
        <p class="hint" style="margin:6px 0 0;font-size:11px;">${streakHint}</p>
      </div>
    </div>
  </div>`;
  // 동네랭킹·누적성과·크루 유도 — 이제 캐릭터/출석과 같은 줄이 아니라 그 아래 별도의 3열
  // 줄로 표시하고, 운동 히스토리 카드보다는 위에 온다.
  const glanceRow3 = `
  <div class="grid glance-row-3" style="align-items:stretch;margin-top:14px;">
  ...
  <div style="margin-bottom:16px;">${renderGuestBlur(glanceGrid + glanceRow3, '로그인하면 내 캐릭터·출석·랭킹을 볼 수 있어요')}</div>`;
