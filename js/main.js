...
// [담당] 로그인 후 첫 화면(요약 대시보드) — 순위/등급/크루 요약 카드.
// [백엔드 연동] 자체 fetch는 거의 없다. auth.js/profile.js 등이 로그인 시 이미 불러온 state
//              값(state.user, state.rank 등)을 재사용해서 그리기만 한다.
// [주의] 실제 이력 데이터가 없어 "어제 대비 변화"류 화살표는 만들지 않았다(가짜 데이터였던
//        부분은 이번 세션에 정리됨) — 새 통계 카드 추가 시 실제 서버 값인지 꼭 확인할 것.
