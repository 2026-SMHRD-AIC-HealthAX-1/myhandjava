...
async function setMenu(id){
  ...
  if(state.menu==='crew' && id!=='crew') { disconnectCrewChat(); disconnectJoinWait(); }
  state.menu=id;
  saveSessionMenu(id); // 새로고침하면 마지막으로 보던 메뉴로 돌아오도록 기억해둔다
  render();
  if(id==='crew'){
    await loadMyCrew();
    if(!state.crew.created){ await loadJoinableCrews(); connectJoinWait(); } // 가입 신청 대기 중일 수 있으니 승인 알림을 받을 소켓을 연결해둔다
    else { connectCrewChat(); } // 크루 공지 기능은 정책 개편으로 제거
    render();
  } else if(id==='support'){
...
function goHome() { if(state.menu==='crew') { disconnectCrewChat(); disconnectJoinWait(); } state.menu = 'main'; saveSessionMenu('main'); render(); }
...
function renderConfirm() {
  ...
  <div class="confirm-backdrop" onclick="if(event.target===this)closeConfirm()">
    <div class="confirm-box">
      <h3>${c.title}</h3>
      <p style="white-space:pre-line;">${c.desc}</p>
