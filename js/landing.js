...
// [담당] 로그인 전 첫 화면 + 비회원(게스트) 체험 진입.
// [백엔드 연동] 없음 — 게스트 모드는 로그인 없이 state.guestMode=true로 로그인 화면과 동일한
//              UI를 보여줄 뿐, 실제 계정에 남는 액션은 로그인으로 유도한다.
// [주의] state.user는 로그인 여부와 무관하게 항상 존재하는 공유 객체라, 로그아웃 시 필드를
//        제대로 안 지우면 다음 게스트/계정에 이전 값이 새어나갈 수 있다(profile.js doLogout 참고).
...
function renderLandingBottomNav(){
  const items=[
    {icon:'<img src="assets/오운홈 로고.png" alt="오운홈" style="width:24px;height:24px;object-fit:contain;vertical-align:middle;">', label:'홈', action:"window.scrollTo({top:0,behavior:'smooth'})"},
  ...
  <div class="landing-shell">
    ...
    <div class="landing-body">
      <div class="landing-feature-banner">
        <h2 class="landing-section-title"><span class="landing-title-rays" aria-hidden="true"></span><span>이런 걸 할 수 있어요</span><span class="landing-title-rays" aria-hidden="true"></span></h2>
        <p>오운홈과 함께, 더 건강하고 즐거운 일상을 시작해요!</p>
      </div>
