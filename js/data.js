...
const OAUTH_REDIRECT_URI =
  'https://lbmp3mgt-5500.jpe1.devtunnels.ms/index.html';
...
const SESSION_GENDER_KEY = 'ounhome_gender';
...
function normalizeGender(value, fallback = 'male') {
  ...
  if (['female', 'f', 'woman', '여성', '여자'].includes(normalized)) return 'female';
  if (['male', 'm', 'man', '남성', '남자'].includes(normalized)) return 'male';
  return fallback === 'female' ? 'female' : 'male';
}
function saveSessionGender(gender) {
  try { localStorage.setItem(SESSION_GENDER_KEY, normalizeGender(gender)); } catch (err) { }
}
function loadSessionGender() {
  try { return normalizeGender(localStorage.getItem(SESSION_GENDER_KEY), 'male'); } catch (err) { return 'male'; }
}
...
function clearSession() {
  try { localStorage.removeItem(SESSION_TOKEN_KEY); localStorage.removeItem(SESSION_MENU_KEY); localStorage.removeItem(SESSION_GENDER_KEY); } catch (err) { }
}
