...
/* ========================================================================
   ...
   ======================================================================== */
// (FR-EX-001~004) 종목 선택 ~ 스마트폰 카메라 촬영(startSkeletonLoop, toggleRecording)까지는
// 브라우저에서 도는 촬영·자세 인식 로직이라 프론트엔드에 그대로 남습니다.
//   스마트폰 카메라 스트림(JS) > (실제 구현 시) MediaPipe Pose 실시간 분석(WASM) > 관절 각도·등급 계산(JS)
...
// 리플레이 분석은 더 이상 별도 단계가 아니다 — 스마트폰 카메라 촬영이 끝나면 그 화면 위에 바로 팝업으로
// 뜬다(renderReplayPopup 참고). 촬영 중이던 카메라 화면 그대로 뒤에 남아있는 게 자연스러워서
// 단계를 옮기지 않고 팝업으로 처리했다.
const EX_STEPS = ['종목 선택', '튜토리얼', '스마트폰 카메라 촬영', '결과 저장'];
...
// 튜토리얼을 최소 몇 초는 보게 한 뒤에 "스마트폰 카메라 촬영 시작" 버튼을 눌러 다음 단계로 넘어갈 수
...
function startTutorialGate() {
  ...
  if (!btn) return;
  btn.textContent = `스마트폰 카메라 촬영 시작 (${left}초)`;
  const tick = () => {
    const b = document.getElementById('ex-tutorial-start-btn');
    if (!b) return; // 다른 화면으로 이동하면 자연 종료
    left--;
    if (left > 0) {
      b.textContent = `스마트폰 카메라 촬영 시작 (${left}초)`;
      setTimeout(tick, 1000);
    } else {
      b.textContent = '스마트폰 카메라 촬영 시작';
  ...
  <div class="grid grid-2">
    <div class="card">
      ...
      <div style="margin-top:16px;display:flex;gap:8px;">
        ...
        <button class="btn btn-primary" id="ex-tutorial-start-btn" disabled style="opacity:.5;cursor:not-allowed;" onclick="goExStep(2)">스마트폰 카메라 촬영 시작 (${TUTORIAL_GATE_SECONDS}초)</button>
      </div>
    </div>
...
function startPoseFeedback() {
  ...
  // 튜토리얼에서 "스마트폰 카메라 촬영 시작"을 누르고도 다음 화면에서 "촬영 시작"을 또 눌러야 하는 게
...
/* ---------- 스쿼트 실시간 자세 판정 (MediaPipe Pose, 실제 스마트폰 카메라) ---------- */
...
function exRegisterRep(bottomAngle, torsoDrop) {
  ...
  const weight = { PERFECT: 100, GREAT: 80, GOOD: 50, MISS: 0 };
...
function toggleRecording() {
  ...
  if (state.exercise.camPhase === 'idle') {
    ...
    // 각도까지 잘 보이도록 스마트폰 카메라 화면을 한 단계 더 키운다(모바일 전용, style.css 참고).
    const stage = document.getElementById('cam-stage');
...
function beginRecording() {
  ...
  state.exercise.timerId = setInterval(() => {
    ...
    const remainEl = document.getElementById('exercise-time-left');
    if (remainEl) remainEl.textContent = `${String(Math.floor(Math.max(0, EXERCISE_TIME_LIMIT_SECONDS-state.exercise.seconds)/60)).padStart(2,'0')}:${String(Math.max(0, EXERCISE_TIME_LIMIT_SECONDS-state.exercise.seconds)%60).padStart(2,'0')}`;
    if (state.exercise.seconds >= EXERCISE_TIME_LIMIT_SECONDS) {
      stopRecording();
      return;
    }
...
function stopRecording() {
  ...
  const finish = () => {
    ...
    // 리플레이 팝업으로 바뀐 뒤에도 스마트폰 카메라 촬영 화면(cam-canvas)이 그대로 DOM에 남아있어서
    // (자연 종료 조건이던 "cam-canvas가 사라짐"이 더 이상 발생하지 않는다), 실시간 포즈
    // 인식 루프를 명시적으로 멈추지 않으면 화면 뒤에서 계속 빈 프레임을 돌게 된다.
    cancelAnimationFrame(exRAF);
    generateResult();
    // 예전엔 별도의 "리플레이 분석" 단계(step 3)로 넘어갔지만, 지금은 스마트폰 카메라 촬영 화면에 그대로
    // 남은 채로 결과를 팝업으로 띄운다 — 단계(step)는 그대로 2(스마트폰 카메라 촬영)를 유지한다.
...
function generateResult() {
  if (state.exercise.picked === 'squat' && state.exercise.liveReps.length > 0) {
    ...
    const score = reps.reduce((sum, rep) => sum + weight[rep.grade], 0);
  ...
  const valid = total - missCount;
  const acc = Math.round((reps.reduce((s, r) => s + (r.grade === 'PERFECT' ? 100 : r.grade === 'GREAT' ? 80 : r.grade === 'GOOD' ? 50 : 0), 0)) / total);
  const score = reps.reduce((s, r) => s + (r.grade === 'PERFECT' ? 100 : r.grade === 'GREAT' ? 80 : r.grade === 'GOOD' ? 50 : 0), 0);
  const ex = EXS.find(e => e.id === state.exercise.picked) || EXS[0];
...
// 예전엔 "리플레이 분석"이 스마트폰 카메라 촬영 다음의 별도 단계(step)였지만, 촬영이 끝나면 카메라
...
async function saveExerciseResult() {
  ...
  try {
    ...
    r.pointsAwarded = Number(body.data.pointsAwarded ?? 0);
    r.highScoreUpdated = !!body.data.highScoreUpdated;
