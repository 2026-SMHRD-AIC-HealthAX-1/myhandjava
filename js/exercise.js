// exercise.js — '운동' 카테고리: 종목 선택 → 튜토리얼 → 카메라 촬영(실시간 자세 판정) → 결과 저장. 가장 큰 파일입니다.

/* ========================================================================
   1. 운동 (EXERCISE WIZARD)
   ======================================================================== */
// (FR-EX-001~004) 종목 선택 ~ 카메라 촬영(startSkeletonLoop, toggleRecording)까지는
// 브라우저에서 도는 촬영·자세 인식 로직이라 프론트엔드에 그대로 남습니다.
//   카메라 스트림(JS) > (실제 구현 시) MediaPipe Pose 실시간 분석(WASM) > 관절 각도·등급 계산(JS)
...
// 리플레이 분석은 더 이상 별도 단계가 아니다 — 카메라 촬영이 끝나면 그 화면 위에 바로 팝업으로
// 뜬다(renderReplayPopup 참고). 촬영 중이던 카메라 화면 그대로 뒤에 남아있는 게 자연스러워서
// 단계를 옮기지 않고 팝업으로 처리했다.
const EX_STEPS = ['종목 선택', '튜토리얼', '카메라 촬영', '결과 저장'];
...
const EXERCISE_REP_TARGET = 15; // 실시간 판정이 지원되는 운동(스쿼트)의 세션(세트)당 반복 횟수 — 백엔드 ExerciseScorePolicy.MAX_REPS와 동일
const EXERCISE_MAX_SECONDS = 120; // 한 세트 최대 시간(2분) — 백엔드 ExerciseSession.MAX_DURATION_SECONDS와 동일. 다 채우지 못해도 이 시간이 지나면 자동 종료된다.
// 자세 판정 등급별 점수 — 백엔드 ExerciseScorePolicy와 동일한 값을 프론트에서도 그대로 써서
// 실시간 화면에 보이는 점수와 저장 후 서버가 계산하는 점수가 같게 맞춘다.
const GRADE_POINTS = { PERFECT: 100, GREAT: 80, GOOD: 50, MISS: 0 };
// 결과 저장 화면에서 "기록 저장"을 누르기 전에 획득 경험치를 미리 보여주기 위해 백엔드
// ExerciseExpPolicy.calculateExp()과 동일한 구간표를 그대로 옮겨왔다 — 실제 지급값은
// 저장 응답의 expAwarded를 신뢰하고, 이 값은 저장 전 미리보기 용도로만 쓴다.
function previewExerciseExp(score) {
  if (score >= 1500) return 500;
  if (score >= 1350) return 450;
  if (score >= 1200) return 400;
  if (score >= 1050) return 350;
  if (score >= 900) return 300;
  if (score >= 750) return 250;
  if (score >= 500) return 150;
  if (score >= 250) return 50;
  return 0;
}
// 운동 결과 저장 API의 중복 방지 키 — 같은 요청이 두 번 전송돼도(네트워크 재시도 등) 서버가
// 같은 결과를 두 번 반영하지 않도록 매 저장 시도마다 새로 만든다.
function generateIdempotencyKey() {
  if (window.crypto && window.crypto.randomUUID) return window.crypto.randomUUID();
  return `ex-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
// 하루에 완료할 수 있는 운동세트(세션) 한도 — 기본 3세트, 포인트 상점 "운동 추가권" 1개
// 구매마다 +3세트. state.user.setsUsedToday는 saveExerciseResult()에서 세션을 저장할 때마다
// 늘어난다(재촬영은 이미 FREE_RETAKES/티켓으로 따로 제한되므로 여기 세지 않음).
const EXERCISE_DAILY_SETS_BASE = 3;
function getDailySetLimit() { return EXERCISE_DAILY_SETS_BASE + (state.user.extraSets || 0); }
    ...
    <div class="card" style="margin-bottom:16px;">
      <div class="flex-between" style="align-items:flex-start;gap:12px;">
        <div style="flex:1;min-width:0;">
          ...
          ${noSetsLeft ? `<p class="hint" style="margin-top:6px;">'운동 추가권'을 구매하면 오늘 바로 더 운동할 수 있어요.</p>` : `<p class="hint" style="margin-top:6px;">'운동 추가권' 구매로 한도를 늘릴 수 있어요.</p>`}
        </div>
        <div style="flex:none;text-align:center;">
          <div class="ex-badge" style="width:46px;height:46px;font-size:22px;margin:0 auto;">🎟️</div>
          <button class="btn btn-sm btn-primary" style="margin-top:6px;white-space:nowrap;" onclick="openExtraSetPurchase()">구매</button>
        </div>
      </div>
    </div>
...
// state.exercise의 초기 모양을 한 곳에서 관리한다 — 운동 위저드 초기화 지점이 여러 파일에
// 흩어져 있어서(크루대전 진입/종료 포함) sessionId 같은 필드를 새로 추가할 때 하나라도
// 빠뜨리면 이전 세션의 sessionId가 새 시도에 그대로 남는 버그가 생기기 쉽다.
function freshExerciseState(overrides) {
  return Object.assign({
    step: 0, picked: null, camPhase: 'idle', camStream: null, timerId: null,
    seconds: 0, result: null, retakesUsed: 0, liveReps: [], replayOpen: false,
    sessionId: null, rewardEligible: true, ticketUsed: false,
  }, overrides || {});
}
...
/* ---------- 운동 세션(백엔드) : 하루 무료 횟수·다시찍기 티켓 차감, 120초 제한시간 ----------
   생성(createSession, 미차감) → 시작(startSession, 여기서 무료 횟수/티켓 차감 + reward
   가능 여부 확정) → 결과 저장(saveExerciseResult)으로 이어지는 서버 세션 생명주기를 그대로
   따라간다. 게스트 모드는 서버 세션이 없으므로 모든 함수가 조용히 아무 일도 하지 않는다. */
async function createExerciseSession(exerciseType) {
  if (state.guestMode || !state.token) return null;
  try {
    ...
    if (!body.success) { toast(body.message || '운동 세션 생성에 실패했습니다'); return null; }
...
async function startExerciseSession(sessionId) {
  ...
  try {
    ...
    const body = await res.json();
    if (!body.success) { toast(body.message || '운동을 시작할 수 없습니다'); return null; }
    return body.data;
  } catch (err) {
    toast('서버에 연결할 수 없습니다 (백엔드가 켜져 있는지 확인해주세요)');
    return null;
...
// 정렬 중(ready)이거나 촬영 중(recording)인데 "이전"으로 빠져나가는 경우 — 이미 차감된
// 무료 횟수/티켓은 복구되지 않지만, 세션 상태만이라도 서버에 정리해둔다.
async function abortExerciseSession(sessionId) {
  if (!sessionId) return;
  try {
    await fetch(`${API_BASE}/api/exercise-sessions/${sessionId}/abort`, {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + state.token },
    });
  } catch (err) { /* best-effort 정리라 실패해도 사용자 흐름은 막지 않는다 */ }
}
function goExStep(n) {
  state.exercise.step = n; render();
  if (n === 1) startTutorialGate(); // 튜토리얼에 들어올 때마다(뒤로 왔다가 다시 와도) 대기시간을 새로 건다
}
// 카메라 화면에서 "이전"을 누른 경우 — 정렬 중/촬영 중이었다면(세션이 이미 생성됐다면)
// 서버 세션을 중도 종료 처리하고, 카메라도 꺼준다.
function leaveExerciseCamStep() {
  if (state.exercise.sessionId && !state.exercise.result) abortExerciseSession(state.exercise.sessionId);
  ...
  state.exercise.sessionId = null;
  goExStep(1);
}
// 튜토리얼을 최소 몇 초는 보게 한 뒤에 "카메라 촬영 시작" 버튼을 눌러 다음 단계로 넘어갈 수
...
function goToTutorial() {
  ...
  if (!state.guestMode && (state.user.setsUsedToday || 0) >= getDailySetLimit()) {
    toast(`오늘 가능한 운동세트를 모두 사용했어요 (${getDailySetLimit()}세트). 포인트 상점에서 운동 추가권을 구매하거나 내일 다시 시도해주세요.`);
    return;
  }
  goExStep(1);
}
  ...
  <div class="grid grid-2">
    <div class="card">
      ...
      <div style="margin-top:16px;display:flex;gap:8px;">
        ...
        <button class="btn btn-primary" id="ex-tutorial-start-btn" disabled style="opacity:.5;cursor:not-allowed;" onclick="goExStep(2)">카메라 촬영 시작 (${TUTORIAL_GATE_SECONDS}초)</button>
      </div>
    </div>
  ...
  <div class="grid cal-grid">
    <div>
      ...
      : `<button class="btn btn-primary" id="cam-toggle" onclick="toggleRecording()">촬영 시작</button>`}
        <button class="btn btn-ghost" onclick="leaveExerciseCamStep()">이전</button>
      </div>
    </div>
    <div class="card">
      ...
      ? `<li><span class="num">·</span><span><strong>카메라는 12시 방향에 두고, 다리 방향은 2시 방향을 향하도록 살짝 틀어 서주세요.</strong> (각도 판정 정확도를 위해 정면보다는 대각선 자세가 필요해요)</span></li>
             ...
             <li><span class="num">·</span>${EXERCISE_REP_TARGET}회를 채우면 자동으로 촬영이 종료되고, 다 채우지 못해도 <strong>${EXERCISE_MAX_SECONDS / 60}분</strong>이 지나면 자동으로 종료됩니다.</li>`
...
function setupCamera() {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false }).then(stream => {
      ...
      const ph = document.getElementById('cam-placeholder');
      if (ph) ph.innerHTML = '카메라를 사용할 수 없습니다.<br>카메라 프리뷰 없이 모의 자세 인식으로 진행합니다.';
      startPoseFeedback();
    });
  } else {
    const ph = document.getElementById('cam-placeholder');
...
function startPoseFeedback() {
  ...
  // 튜토리얼에서 "카메라 촬영 시작"을 누르고도 다음 화면에서 "촬영 시작"을 또 눌러야 하는 게
...
/* ---------- 스쿼트 실시간 자세 판정 (MediaPipe Pose, 실제 카메라) ---------- */
...
let exRAF = null;
let exSessionPromise = null; // createExerciseSession()의 진행 중인 Promise — beginRecording()이 촬영 시작 시점에 sessionId를 기다릴 때 쓴다
...
function exRegisterRep(bottomAngle, torsoDrop) {
  ...
  const acc = Math.round(state.exercise.liveReps.reduce((s, r) => s + GRADE_POINTS[r.grade], 0) / state.exercise.liveReps.length);
...
function toggleRecording() {
  ...
  if (state.exercise.camPhase === 'idle') {
    ...
    // 각도까지 잘 보이도록 카메라 화면을 한 단계 더 키운다(모바일 전용, style.css 참고).
    const stage = document.getElementById('cam-stage');
    if (stage) stage.classList.add('cam-grown');
    if (isSquat) {
      // 정렬·카운트다운 몇 초 동안 서버에 세션을 미리 만들어둔다 — 아직 무료 횟수/티켓을
      // 차감하지 않는 단계라(백엔드 createSession 주석 참고) 실제 촬영 시작(beginRecording)
      // 전에 미리 준비해둬도 안전하다.
      const exName = EXS.find(e => e.id === 'squat').name;
      exSessionPromise = createExerciseSession(exName).then(id => { state.exercise.sessionId = id; return id; });
      startAlignmentGuide();
    } else beginRecording();
    return;
  }
...
async function beginRecording() {
  ...
  // "촬영 시작" 버튼을 누른 시점(정렬 전)에 만들어둔 세션을 여기서 실제로 시작 처리한다 —
  // 무료 횟수/다시찍기 티켓 차감과 reward 가능 여부(rewardEligible/ticketUsed)가 이 시점에 확정된다.
  if (isSquat) {
    const sessionId = state.exercise.sessionId || (exSessionPromise ? await exSessionPromise : null);
    const started = await startExerciseSession(sessionId);
    if (started) {
      state.exercise.sessionId = sessionId;
      state.exercise.rewardEligible = started.rewardEligible;
      state.exercise.ticketUsed = started.ticketUsed;
    }
  }
  ...
  state.exercise.timerId = setInterval(() => {
    ...
    // 스쿼트는 남은 시간을 보여주는 카운트다운(2분 → 0), 그 외 운동은 지금까지 흐른 시간을
    // 그대로 보여준다(별도의 제한시간이 없음).
    const displaySeconds = isSquat ? Math.max(0, EXERCISE_MAX_SECONDS - state.exercise.seconds) : state.exercise.seconds;
    const m = String(Math.floor(displaySeconds / 60)).padStart(2, '0');
    const s = String(displaySeconds % 60).padStart(2, '0');
    if (timerEl) timerEl.textContent = `${m}:${s}`;
    if (isSquat && state.exercise.seconds >= EXERCISE_MAX_SECONDS) { toggleRecording(); return; } // 15회를 못 채워도 2분이 지나면 자동 종료
...
function stopRecording() {
  ...
  const finish = () => {
    ...
    if (state.exercise.camStream) { state.exercise.camStream.getTracks().forEach(t => t.stop()); state.exercise.camStream = null; }
    // 리플레이 팝업으로 바뀐 뒤에도 카메라 촬영 화면(cam-canvas)이 그대로 DOM에 남아있어서
    // (자연 종료 조건이던 "cam-canvas가 사라짐"이 더 이상 발생하지 않는다), 실시간 포즈
    // 인식 루프를 명시적으로 멈추지 않으면 화면 뒤에서 계속 빈 프레임을 돌게 된다.
    cancelAnimationFrame(exRAF);
    generateResult();
    // 예전엔 별도의 "리플레이 분석" 단계(step 3)로 넘어갔지만, 지금은 카메라 촬영 화면에 그대로
    // 남은 채로 결과를 팝업으로 띄운다 — 단계(step)는 그대로 2(카메라 촬영)를 유지한다.
...
function generateResult() {
  if (state.exercise.picked === 'squat' && state.exercise.liveReps.length > 0) {
    ...
    reps.forEach(r => gc[r.grade]++);
    const total = reps.length;
    const valid = total - gc.MISS;
    // 총점 = 등급별 점수 합(백엔드 ExerciseScorePolicy.calculateScore와 동일한 계산) —
    // 한 렙의 최대 점수가 100점이라 자연스럽게 평균이 곧 정확도(%)가 된다.
    const score = reps.reduce((s, r) => s + GRADE_POINTS[r.grade], 0);
    const acc = total ? Math.round(score / total) : 0;
    ...
    // 지난 스쿼트 기록(state.history, 로그인 시 미리 불러와둠) 중 최고 점수보다 이번 점수가
    // 높으면 "최고점" — 대상이 없으면(첫 시도) 무조건 최고점으로 표시한다.
    const priorBest = state.history.filter(h => h.ex === '스쿼트').reduce((max, h) => Math.max(max, h.score), -1);
    const isBest = score > priorBest;
    state.exercise.result = {
      ex: ex.name, dur, total, valid,
      missCount: gc.MISS, perfectCount: gc.PERFECT, greatCount: gc.GREAT, goodCount: gc.GOOD,
      acc, score, reps, compareRep, isBest,
      sessionId: state.exercise.sessionId, rewardEligible: state.exercise.rewardEligible, ticketUsed: state.exercise.ticketUsed,
      myVideoUrl: state.exercise._pendingVideoUrl || null,
    };
...
// 예전엔 "리플레이 분석"이 카메라 촬영 다음의 별도 단계(step)였지만, 촬영이 끝나면 카메라
...
function retakeExercise() {
  ...
  // 방금 촬영한 세션은 저장하지 않고 버리는 것이므로 서버에도 중도 종료로 정리해둔다 —
  // 다음 "촬영 시작"을 누르면 toggleRecording()이 새 세션을 만들어 sessionId를 덮어쓴다.
  if (ex.sessionId) abortExerciseSession(ex.sessionId);
  ex.sessionId = null; ex.rewardEligible = true; ex.ticketUsed = false;
  ex.result = null;
  ex.liveReps = [];
  ex.replayOpen = false;
  goExStep(2);
...
function renderExStepSave() {
  ...
  const isSquat = state.exercise.picked === 'squat';
  const ticketUsed = isSquat && !!r.ticketUsed;
  const expPreview = ticketUsed ? 0 : previewExerciseExp(r.score);
  const pointsPreview = ticketUsed ? 0 : Math.round(r.score * 0.4);
  // r.perfectCount 등은 스쿼트(실제 판정) 결과에만 있으므로, 어떤 운동이든 reps 배열에서
  // 직접 세어 항상 정확한 값을 쓴다.
  const gc = { PERFECT: 0, GREAT: 0, GOOD: 0, MISS: 0 };
  (r.reps || []).forEach(rp => { if (gc[rp.grade] != null) gc[rp.grade]++; });
  return `
  <div class="card" style="max-width:560px;margin:0 auto;">
    <div class="flex-between" style="align-items:flex-start;">
      <p class="section-label" style="margin:0;">획득 요약</p>
      ${r.isBest ? `<span class="pill pill-gold">🏆 최고점</span>` : ''}
    </div>
    <h3 style="font-size:20px;">${r.ex} 세션 완료 ${gradePill(r.acc > 90 ? 'PERFECT' : r.acc > 78 ? 'GREAT' : r.acc > 60 ? 'GOOD' : 'MISS')}</h3>
    <p class="desc mono" style="margin:0 0 12px;">1세트 · ${r.total}회 수행</p>
    <div class="stat-row">
      <div class="stat-box"><div class="num mono" style="color:${gradeColor('PERFECT')}">${gc.PERFECT}</div><div class="lbl">PERFECT</div></div>
      <div class="stat-box"><div class="num mono" style="color:${gradeColor('GREAT')}">${gc.GREAT}</div><div class="lbl">GREAT</div></div>
      <div class="stat-box"><div class="num mono" style="color:${gradeColor('GOOD')}">${gc.GOOD}</div><div class="lbl">GOOD</div></div>
      <div class="stat-box"><div class="num mono" style="color:${gradeColor('MISS')}">${gc.MISS}</div><div class="lbl">MISS</div></div>
    </div>
    <div class="stat-row" style="margin-top:8px;">
      <div class="stat-box"><div class="num mono" style="color:var(--gold)">${r.score}</div><div class="lbl">총점</div></div>
      <div class="stat-box"><div class="num mono" style="color:var(--gold)">+${expPreview}</div><div class="lbl">획득 경험치</div></div>
      <div class="stat-box"><div class="num mono" style="color:var(--gold)">+${pointsPreview}</div><div class="lbl">포인트</div></div>
    </div>
    ${isSquat ? `<p class="hint" style="margin-top:10px;">${ticketUsed
      ? '🎫 다시찍기 티켓을 사용한 운동이에요. 기록·최고점에는 반영되지만 경험치·포인트는 지급되지 않아요.'
      : '✅ 오늘의 무료 운동 기회로 진행했어요. 경험치와 포인트가 정상 지급돼요.'}</p>` : ''}
    ${state.guestMode ? `
    <p class="hint" style="margin:10px 0;">게스트 모드에서는 기록이 저장되지 않아요. 로그인하면 방금 결과부터 정식으로 기록·포인트 적립이 시작돼요.</p>
    ...
    ` : `<button class="btn btn-primary btn-block" style="margin-top:10px;" onclick="saveExerciseResult()">기록 저장</button>`}
  </div>`;
}
...
async function saveExerciseResult() {
  ...
  try {
    ...
    const exp = body.data.expAwarded;
    ...
    // 다시찍기 티켓을 사용한 세션(rewardEligible=false)은 서버가 경험치·포인트를 0으로
    // 저장하므로, 토스트에서도 그 사실을 그대로 보여준다.
    toast(body.data.rewardEligible === false
      ? '저장 완료! (다시찍기 티켓 사용 · 경험치·포인트 없음)'
      : `저장 완료! +${exp} EXP · +${pts}P 획득`);
