...
function renderMissionAvatar() {
  ...
  return `
  <div class="grid grid-fixed-2 profile-top-grid">
    <div class="card" style="text-align:center;">
      <div class="profile-char-grid">
        <div style="min-width:0;">
          <p class="section-label">내 캐릭터</p>
          <canvas id="avatar-char-canvas" style="width:100%;max-width:210px;height:auto;aspect-ratio:210/256;margin:10px auto;display:block;border-radius:10px;image-rendering:pixelated;"></canvas>
          <h3 style="color:${nickColor};margin:0;">${state.user.nickname || '홈트초보'}</h3>
          ${userLevelBadge(state.user.grade, state.user.gradeName, state.user.level)}
        </div>
        <div class="field" style="min-width:0;margin:0;text-align:left;">
          <label for="profile-bio-input">자기소개</label>
          <textarea id="profile-bio-input" rows="11" maxlength="80" style="resize:none;" placeholder="나를 소개하는 한마디를 남겨보세요">${state.user.bio || ''}</textarea>
...
function getEquipState() {
  // 슬롯별로 'true'가 아니라 실제 아이템 객체를 전달합니다.
  // 그래야 drawPixelCharacter()가 해당 아이템의 이미지/크기/위치를 사용할 수 있습니다.
  const bySlot = {};
  state.shopItems.forEach(it => {
    if (it.slot && it.owned && it.equipped) bySlot[it.slot] = it;
  });
  return bySlot;
}
...
const EQUIPMENT_IMAGE_CACHE = {};

function loadEquipmentImage(src) {
  if (!src) return null;
  if (EQUIPMENT_IMAGE_CACHE[src]) return EQUIPMENT_IMAGE_CACHE[src];

  const img = new Image();
  img.onload = () => {
    // 비동기 이미지 로딩이 끝나면 현재 보이는 캐릭터들을 다시 그립니다.
  ...
  img.onerror = () => console.warn('아이템 이미지를 불러올 수 없습니다:', src);
  img.src = src;
  EQUIPMENT_IMAGE_CACHE[src] = img;
  return img;
}

function drawEquipmentItem(ctx, canvas, item, gender) {
  if (!item || !item.image) return;

  const sex = gender === 'female' ? 'female' : 'male';
  const src = typeof item.image === 'string' ? item.image : item.image[sex];
  if (!src) return;

  const img = loadEquipmentImage(src);
  if (!img || !img.complete || !img.naturalWidth) return;

  const fit = (item.fit && (item.fit[sex] || item.fit)) || {x:0, y:0, w:1, h:1};
  const x = canvas.width * (fit.x ?? 0);
  const y = canvas.height * (fit.y ?? 0);
  const w = canvas.width * (fit.w ?? 1);
  const h = canvas.height * (fit.h ?? 1);
...
// gender: 'male' | 'female'
// 기본 캐릭터 PNG 위에 아이템 PNG를 레이어로 겹쳐 실제 착용처럼 표시합니다.
// 아이템 PNG는 캐릭터와 동일한 1145x1374 캔버스 기준으로 제작할 수 있으며,
// fit.x/y/w/h를 0~1 비율로 지정하면 화면 크기가 바뀌어도 함께 확대/축소됩니다.
function drawPixelCharacter(canvas, equip = {}, gender = 'male') {
  const W = 1145;
  const H = 1374;
  canvas.width = W;
  canvas.height = H;
  ...
  if (equip.background) {
    ...
    g.addColorStop(0, '#3b2f63');
    g.addColorStop(0.55, '#c06b4f');
    g.addColorStop(1, '#f0b35c');
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);
  }

  const sex = gender === 'female' ? 'female' : 'male';
  const sprite = CHAR_SPRITES[sex];

  if (sprite) {
    ctx.save();

    // 로봇 코치 스킨: 전용 캐릭터 이미지가 없는 경우 색상 필터로 표현
    if (equip.skin) {
      ctx.filter = 'grayscale(0.6) sepia(0.35) hue-rotate(165deg) saturate(2.4)';
    }
    ...
    const dw = sw * scale;
    const dh = sh * scale;
    ctx.drawImage(sprite, (W - dw) / 2, H - dh, dw, dh);
    ctx.restore();
  }

  // 실제 착용 레이어. 숫자가 낮은 아이템부터 그립니다.
  const items = Object.values(equip)
    .filter(it => it && typeof it === 'object' && it.image)
    .sort((a, b) => (a.layer || 0) - (b.layer || 0));

  items.forEach(item => drawEquipmentItem(ctx, canvas, item, sex));

  // 기존 데이터와의 호환: 이미지가 없는 왕관은 기존 벡터 왕관으로 표시
  if (equip.crown && !equip.crown.image) {
    const xL = W * 0.30;
    const xR = W * 0.70;
    const baseY = H * 0.15;
    const topY = H * 0.035;
  ...
  // 기존 데이터와의 호환: badge 이미지가 없으면 프레임으로 표시
  if (equip.badge && !equip.badge.image) {
    ctx.strokeStyle = '#D9A226';
...
async function loadMyCrew() {
  ...
  try {
    ...
    state.crew.concepts = c.concepts || [];
...
// 크루 메인의 "크루대전 누적 점수" 카드용 — 크루원 전체가 아니라 실제로 크루대전에
// 참여해 점수를 쌓은 크루원만 내려온다(GET /api/crews/me/battle-contributions).
async function loadCrewBattleContributions() {
  ...
  try {
    ...
    const body = await res.json();
    if (!body.success) return;
    state.crew.battleContributions = body.data.map(c => ({
      userId: c.userId, nickname: c.nickname, totalScore: c.totalScore, currentMember: c.currentMember,
    }));
  } catch (err) {
    console.error('크루대전 누적 점수 불러오기 실패', err);
  }
}
