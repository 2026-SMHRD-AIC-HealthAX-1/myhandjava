...
// 크루 컨셉은 화면(setCrewConcept)에서 최대 3개까지 고를 수 있게 해놨지만, DB엔 concept
// 컬럼이 문자열 하나뿐이라(생성 때도 createCrew()에서 concepts[0]만 보냄) 저장도 똑같이
// 첫 번째로 고른 컨셉만 반영된다 — 여러 개를 동시에 저장하려면 스키마부터 바꿔야 한다.
async function updateCrewDesc() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/crews/me`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + state.token },
      body: JSON.stringify({ description: v, concept: state.crew.concepts[0] })
    });
    const body = await res.json();
    if (!body.success) { toast(body.message || '저장에 실패했습니다'); return; }
    state.crew.desc = body.data.description;
    state.crew.concepts = [body.data.concept];
    toast('크루 소개를 저장했습니다');
    render();
  } catch (err) {
    toast('서버에 연결할 수 없습니다');
